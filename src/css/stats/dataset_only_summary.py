...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dataset-only stats: correlate CSS metrics with surprisal shifts."
    )
    ...
    for _, key_row in unique_keys.iterrows():
        ...
        if len(g) < 20:
            continue
        y = g["abs_delta_avg_surprisal"].to_numpy(dtype=float)
    ...
    if corr_df.empty:
        raise SystemExit("No dataset-only correlation rows generated")
    ...
    # Frobenius complementarity for surprisal-target prediction.
    ...
    for _, key_row in unique_keys.iterrows():
        ...
        if len(g) < 25:
            continue
        d = g.copy()
        d["z_y"] = _z(d["abs_delta_avg_surprisal"])
    ...
    # Digest
    d1_support = corr_df[
        (corr_df["metric"].isin(["delta_cos", "delta_frob"]))
        & (corr_df["spearman_rho"] > 0)
        & (corr_df["spearman_q"] < 0.05)
    ].shape[0]
    d2_support = h2_df[h2_df["delta_adj_r2"] > 0].shape[0] if not h2_df.empty else 0
    d3_groups = corr_df.groupby(["model", "phenomenon", "metric"]).ngroups
    d4_selectivity_mean = (
        float(probes["selectivity"].mean()) if "selectivity" in probes.columns else float("nan")
    )

    with (out_dir / "hypothesis_tests.md").open("w", encoding="utf-8") as f:
        experiment_name = str(cfg.get("experiment_name", "css_dataset_only_stats"))
        f.write(f"# Dataset-Only Hypothesis Summary ({experiment_name})\n\n")
        f.write(
            f"- D1 significant positive metric-surprisal cells (delta_cos/delta_frob, q<0.05): {d1_support}\n"
        )
        f.write(
            f"- D2 cells with positive delta_adj_r2 when adding delta_frob (surprisal target): {d2_support}\n"
        )
        f.write(f"- D3 layer-profile groups generated: {d3_groups}\n")
        f.write(f"- D4 mean probe selectivity: {d4_selectivity_mean:.4f}\n")
